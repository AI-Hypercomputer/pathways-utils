...
class CloudPathwaysArrayHandler(type_handlers.ArrayHandler):
    ...
    async def deserialize(
        ...
        for arg in args:
            ...
            if arg.sharding is None:
                ...
                if not isinstance(arg.sharding, jax.sharding.NamedSharding):
                    raise ValueError(
                        "Pathways only supports jax.sharding.NamedSharding."
                    )
        ...
        logging.warning(
            f"[ksadi] restore loop will take {len(inputs_by_global_mesh.items())} iterations"
        )
        for i, (global_mesh, idxs) in enumerate(inputs_by_global_mesh.items()):
            ...
            arrays = [
                array_and_future[0] for array_and_future in grouped_arrays_and_futures
            ]
            futures = [
            ...
            grouped_arrays = [array[0] for array in arrays]
...
def register_pathways_handlers(
    ...
    logging.debug("Registering CloudPathwaysArrayHandler (Pathways Persistence API).")
