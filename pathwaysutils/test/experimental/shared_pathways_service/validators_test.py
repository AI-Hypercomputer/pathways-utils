...
class ValidatorsTest(parameterized.TestCase):
  ...
  @parameterized.named_parameters(
      dict(
          testcase_name="with_tag",
          image="gcr.io/project/image:tag",
      ),
      dict(
          testcase_name="with_digest",
          image="gcr.io/project/image@sha256:12345",
      ),
      dict(
          testcase_name="with_tag_and_digest",
          image="gcr.io/project/image:tag@sha256:12345",
      ),
      dict(
          testcase_name="path_with_hyphen",
  ...
  def test_validate_proxy_server_image_success(self, image):
    ...
    validators.validate_proxy_server_image(image)

  @parameterized.named_parameters(
      dict(
          testcase_name="empty_string",
          image="",
          expected_regex="Proxy server image cannot be empty.",
      ),
      dict(
      ...
      dict(
          testcase_name="no_slash",
          image="image:tag",
          expected_regex="Proxy server image 'image:tag' must contain '/'.",
      ),
      dict(
          testcase_name="no_tag_or_digest",
          image="gcr.io/project/image",
          expected_regex=(
              "Proxy server image 'gcr.io/project/image' must contain a tag"
