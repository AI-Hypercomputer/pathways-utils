...
class ReshardTest(parameterized.TestCase):
  ...
  def test_ifrt_reshard_donate(
    ...
    self.assertEqual(args[2], expected_donate)
  ...
  def test_ifrt_reshard_pytree(self):
    ...
    mock_transfer.return_value = [mock.Mock(), mock.Mock()]
    ...
    (arrays, shardings, _), _ = mock_transfer.call_args
    self.assertLen(arrays, 2)
    self.assertLen(shardings, 2)
