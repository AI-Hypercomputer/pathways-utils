...
-   `max_slice_restarts`: Maximum number of slice restarts. Defaults to `1000000` (`1_000_000`) in headless mode and Shared Pathways Service (SPS) mode to keep long-running worker pods resilient to client reconnections and preemption events.
...
-   `pathways_rm_and_worker_image`: Base Docker image for Resource Manager and Worker containers.
-   `pathways_proxy_image`: Base Docker image for Proxy container.

### Properties

-   **`head_job_template`**: Returns the `V1JobTemplateSpec` for the head job.
-   **`worker_job_template`**: Returns the `V1JobTemplateSpec` for the worker job.
-   **`pathways_rm_and_worker_image`**: Returns the configured base Resource Manager and Worker Docker image.
-   **`pathways_proxy_image`**: Returns the configured base Proxy Docker image.
